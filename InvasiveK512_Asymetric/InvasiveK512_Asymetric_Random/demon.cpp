...
// Choose the coordinates of a destination deme, with 0 to 3 directions randomly removed
void choose_grid_square(int old_deme_index, long *idum, int *new_x, int *new_y)
{
    int old_x = deme_ints[XCOORD][old_deme_index];
    int old_y = deme_ints[YCOORD][old_deme_index];

    // Array of possible directions: 0 = right, 1 = left, 2 = up, 3 = down
    int directions[4] = {0, 1, 2, 3};

    // Determine how many directions to remove (randomly between 0 and 3)
    int num_directions_to_remove = (int)(4 * ran1(idum)); // Explicit cast to int

    // Randomly remove directions
    for (int i = 0; i < num_directions_to_remove; i++) {
        int remove_index;
        do {
            remove_index = (int)(4 * ran1(idum)); // Ensure remove_index is an integer
        } while (directions[remove_index] == -1); // Ensure it's a valid direction

        directions[remove_index] = -1; // Mark the direction as removed
    }

    // Collect remaining valid directions
    int valid_directions[4];
    int valid_count = 0;

    for (int i = 0; i < 4; i++) {
        if (directions[i] != -1) {
            valid_directions[valid_count++] = directions[i];
        }
    }

    // If no valid directions remain, return the current deme's coordinates (no movement)
    if (valid_count == 0) {
        *new_x = old_x;
        *new_y = old_y;
        return;
    ...
    // Randomly select one of the remaining valid directions
    int chosen_direction = valid_directions[(int)(valid_count * ran1(idum))]; // Cast to int

    // Assign new coordinates based on the chosen direction
    if (chosen_direction == 0) { // Right
        *new_x = old_x + 1;
        *new_y = old_y;
    }
    else if (chosen_direction == 1) { // Left
        *new_x = old_x - 1;
        *new_y = old_y;
    }
    else if (chosen_direction == 2) { // Up
        *new_x = old_x;
        *new_y = old_y + 1;
    }
    else { // Down
